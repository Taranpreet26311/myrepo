...
#define yyparse         uuparse
#define yylex           uulex
#define yyerror         uuerror
#define yydebug         uudebug
#define yynerrs         uunerrs

#define yylval          uulval
#define yychar          uuchar
...
#ifndef YY_UU_PARSE_TAB_H_INCLUDED
# define YY_UU_PARSE_TAB_H_INCLUDED
/* Debug traces.  */
#ifndef YYDEBUG
# define YYDEBUG 0
#endif
...
#line 173 "parse.tab.c" /* yacc.c:355  */
};
...
extern YYSTYPE uulval;

int uuparse (void);

#endif /* !YY_UU_PARSE_TAB_H_INCLUDED  */
...
#line 194 "parse.tab.c" /* yacc.c:358  */
...
{
  "$end", "error", "$undefined", "NUMBER", "UNUMBER", "STR", "BSTR",
  "WORD", "PAYLOADS", "GLOBAL", "MODULE", "BOOL", "'{'", "'}'", "';'",
  "':'", "$accept", "cfgfile", "section", "glines", "plines", "mlines",
  "g_statement", "p_statement", "m_statement", "multi_line_str",
...
#line 117 "parse.y" /* yacc.c:1646  */
    {
		...
		if ((eptr=scan_optmap((const char *)(yyvsp[-4].ptr), (const char *)string)) != NULL) {
			uuerror(eptr);
		}
...
#line 137 "parse.y" /* yacc.c:1646  */
    {
		...
		if (strcasecmp((yyvsp[-7].ptr), "tcp") == 0) {
			proto=IPPROTO_TCP;
		}
		else if (strcasecmp((yyvsp[-7].ptr), "udp") == 0) {
			proto=IPPROTO_UDP;
		}
		else {
		...
		if ((yyvsp[-4].inum) > 0xFFFF || (yyvsp[-4].inum) < 0) {
			uuerror("payload group out of range");
		}
		else {
			plg=(uint16_t)(yyvsp[-4].inum);
		}

		pbuffer_get(&data);

		if ((yyvsp[-6].inum) > 0xFFFF || (yyvsp[-6].inum) < 0) {
			if ((yyvsp[-6].inum) == -1) {
...
#line 245 "parse.y" /* yacc.c:1646  */
    {
		...
		if ((yyvsp[0].ptr) && strlen((yyvsp[0].ptr))) {
